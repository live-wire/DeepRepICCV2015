...
class RepDetector:
    ...
    def get_boundingbox(self):
        ...
        if framesstd == 0:  # no bb, take full frame
            ...
            for i in range(20):
                frameROIRes[i, :, :] = \
                    scipy.misc.imresize(self.frame_set[i, :, :],
                        size=(50, 50), interp='bilinear')

            frameROIRes = numpy.reshape(frameROIRes, (1,
                    frameROIRes.shape[0] * frameROIRes.shape[1]
                    * frameROIRes.shape[2]))
    ...
    # resize to 50x50
        ...
        frameROIRes = numpy.zeros([20, 50, 50])
        for i in range(20):
            frameROIRes[i, :, :] = scipy.misc.imresize(framesRoi[i, :, :
                    ], size=(50, 50), interp='bilinear')
    ...
    # count
        ...
        self.label_array = numpy.insert(self.label_array, history_num
                - 1, output_label, axis=0)

    # take median of the last 4

        med_out_label = \
            numpy.ceil(numpy.median(self.label_array[history_num
                       - 4:history_num]))
    ...
    # globals....    ....
        global test_set_x, test_set_y, shared_test_set_y
        global in_time, out_time, cooldown_in_time, cooldown_out_time
        global global_counter, winner_stride, cur_state, in_frame_num, actions_counter, classify


    # insert new frame

        self.frame_set = numpy.delete(self.frame_set, 0, axis=0)
        self.frame_set = numpy.insert(self.frame_set, 19, proFrame,
                axis=0)

        if cur_state == state.NO_REP:
            self.do_local_count(classify, True)
        if cur_state == state.IN_REP and winner_stride \
            == self.stride_number:
    ...
    # common to all states
        ...
        self.count_array = numpy.insert(self.count_array, history_num
                - 1, self.rep_count, axis=0)
        self.ent_arr = numpy.delete(self.ent_arr, 0, axis=0)
        self.ent_arr = numpy.insert(self.ent_arr, history_num - 1,
                                    self.cur_entropy, axis=0)
        self.std_arr = numpy.delete(self.std_arr, 0, axis=0)
        self.std_arr = numpy.insert(self.std_arr, history_num - 1,
                                    self.cur_std, axis=0)
        ...
        # if we see good condition for rep, take the counting and move to rep state
            ...
            if self.st_std > norep_std_th and self.st_entropy \
                < norep_ent_th:
        ...
        if cur_state == state.IN_REP and winner_stride \
            == self.stride_number:
            lastSixSorted = numpy.sort(self.ent_arr[history_num
                    - 8:history_num])

        # keep counting while entropy is low enough

            if self.st_std > inrep_std_th and self.st_entropy \
                < inrep_ent_th or lastSixSorted[1] < lastsix_ent_th:
        ...
        # continue counting
            ...
            # calc if and how many global count to rewind
                    ...
                    frames_pass = min(frames_pass,
                            reversed_cnt.shape[0] - 1)
                    new_counter = reversed_cnt[frames_pass]
                    print 'couting rewinded by %i' % (global_counter
                            - new_counter)
...
def draw_str(
    ...
    if color[0] + color[1] + color[2] == 0xFF * 3:
        cv2.putText(
            dst,
            s,
            (x + 1, y + 1),
...
def process_single_frame(frame):
    ...
    frame = scipy.misc.imresize(frame, size=(480, 640),
                                interp='bilinear')
...
def start(inputfile):
    ...
    layer1 = LeNetConvPoolLayer(rng, input=layer0.output,
                                ...
                                layer1_w, layer1_h), filter_shape=(60,
                                flt_channels, 3, 3), poolsize=(2, 2))

    layer2 = LeNetConvPoolLayer(rng, input=layer1.output,
                                image_shape=(batch_size, 60, layer2_w,
                                layer2_h), filter_shape=(90, 60, 3, 3),
                                poolsize=(2, 2))
    layer3_input = layer2.output.flatten(2)

    layer3 = HiddenLayer(rng, input=layer3_input, n_in=90 * layer3_w
                         * layer3_h, n_out=500, activation=T.tanh)

    layer4 = LogisticRegression(input=layer3.output, n_in=500, n_out=8)

    cost = layer4.negative_log_likelihood(y)

    classify = theano.function([index],
                               outputs=layer4.get_output_labels(y),
                               givens={x: test_set_x[index
                               * batch_size:(index + 1) * batch_size],
                               y: test_set_y[index * batch_size:(index
                               + 1) * batch_size]})
    ...
    video_writer = cv2.VideoWriter('../out/live_out.avi', fourcc,
                                   frame_rate, (640, 480))
    ...
    # display result on video................
        ...
        if cur_state == state.IN_REP and (out_time - in_time < 4
                or global_counter < 5):
            draw_str(frame, (20, 120), ' new hypothesis (%d) '
                     % global_counter, orange_color, 1.5)
        if cur_state == state.IN_REP and out_time - in_time >= 4 \
            and global_counter >= 5:
            draw_str(frame, (20, 120), 'action %d: counting... %d'
                     % (actions_counter, global_counter), green_color,
                     2)
        if cur_state == state.COOLDOWN and global_counter >= 5:
            draw_str(frame, (20, 120),
                     'action %d: done. final counting: %d'
    ...
    print "Dhruv", global_counter
    return global_counter

if __name__ == "__main__":
    print "COUNT = ", start("000_rope_beach.mp4" if len(sys.argv) < 2 else sys.argv[1])
